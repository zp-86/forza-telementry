...
export function LapComparison({
    ...
    const sectorRows = useMemo(() => {
        ...
        return rows;
    }, [focusLap.points, otherLap.points, isSingleView, compLap.points]);
                    ...
                    <div className="flex-1 bg-neutral-900/50 border border-neutral-800 rounded-xl flex flex-col overflow-hidden min-h-0">
                        ...
                        <div className="flex-1 overflow-y-auto">
                            ...
                            {sectorRows.map(row => (
                                <div
                                    key={row.index}
                                    onMouseEnter={() => setHoveredGate(row.index)}
                                    onMouseLeave={() => setHoveredGate(null)}
                                    className={`grid ${isSingleView ? 'grid-cols-4' : 'grid-cols-5'} px-4 py-3 border-b border-neutral-800/50 items-center hover:bg-neutral-800/30 transition-colors ${hoveredGate === row.index ? 'bg-amber-900/10' : ''}`}
                                >                                    <div className="text-neutral-400 font-bold text-sm">Gate {row.index}</div>
                                    ...
                                    {!isSingleView && (
                                        <div className={`text-right font-mono font-bold ${row.deltaText === '-' ? 'text-neutral-500' : (row.isWorse ? 'text-red-400' : 'text-emerald-400')}`}>
                                            {row.deltaText === '-' ? '-' : row.deltaText + 's'}
                                        </div>
                                    )}
                                </div>
                            ))}
